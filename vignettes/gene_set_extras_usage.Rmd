...
```{r}
enrichGO(
  gene = hit_data |> filter(cluster==4) |> pull(Gene),
...
```{r}
lapply(all_enrichment_results, volcano_plot, interactive=TRUE)
```
...
Now we're down to just 19 terms which is a lot more manageable.
